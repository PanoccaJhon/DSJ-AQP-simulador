...
public class UL_MotorCycleController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        MotorRb.linearDamping = 0.17f;
        backbodydrag = MotorRb.linearDamping;
        MotorRb.angularDamping = 5.00f;
    }
    //
    ...
    public void WheelAlignment()
    {
        ...
        TotalDownWardForce += MotorRb.linearVelocity.sqrMagnitude;
    ...
    void AIBehaviour()
    {
        ...
        Vector3 locvel = transform.TransformDirection(MotorRb.linearVelocity);
        locvel.x *= 0.000000000001f;
        ...
        if (trafficlightctrl == null)
        {
            ...
            else
            {
                if (TriggerObject != null)
                {
                    ...
                    {
                        if (TriggerObject.GetComponent<Rigidbody>().linearVelocity.magnitude > 10f)
                        {
                            Vector3 move = transform.forward * TriggerObject.GetComponent<Rigidbody>().linearVelocity.magnitude * 0.125f * Time.deltaTime;
                            MotorRb.linearVelocity = move;
                        }
                        else
                        {
                            MotorRb.linearDamping = (RearWheelCol.isGrounded) ? 500f : 0.17f;
                        }
                    }
                    else
                    {
    ...
    public void VehicleSpeed()
    {
        ...
        currentspeed = MotorRb.linearVelocity.magnitude * Pi;
        if (currentspeed > topspeed)
            MotorRb.linearVelocity = (topspeed / Pi) * MotorRb.linearVelocity.normalized;
    }
    #endregion
    //
    //boundries 
    ...
    private void OreintBodyRot()
    {
        if (CurrentSpeed < 0.1f)
        {
            ...
            {
                MotorRb.linearDamping = backbodydrag;
                return;
            }
            if (!LooseControl)
            {
        ...
        else
        {
            ...
            if (!LooseControl)
            {
                if (SeizeRot == true)
                {
                    ...
                    MotorRb.angularDamping = !RearWheel.m_isgrounded || !FrontWheel.m_isgrounded ? 0.01f : 1000f;
                    SeizeRot = false;
                }
                else
                {
    ...
    {
        Vector3 localvel = transform.InverseTransformDirection(MotorRb.linearVelocity);
        if (!IsDrifting)
        {
            localvel.x *= SideGrip;
        }
        ...
        if (Mathf.Abs(Accel) == 0)
        {
            localvel.z *= MotorRb.linearVelocity.magnitude < 2f ? 0.000001f : 1f;
        }
        else
        {
            localvel.z *= 1f;
        ...
        //print(localvel.x);
        MotorRb.linearVelocity = transform.TransformDirection(localvel);
        /*
        if (localvel.x > 0)
        {
            // totalforce.x *= 0f;
    ...
    public void TweakSpeed()
    {
        ...
        float Pi = Mathf.PI * 1.15f;
        float maxrotspeed;
        Vector3 rigidspeed = new Vector3(MotorRb.linearVelocity.x, 0.0f, MotorRb.linearVelocity.z);
        ...
        if (MotorRb.linearVelocity.magnitude > 10)
        {
            if (CurrentSpeed > TopSpeed)
                MotorRb.linearVelocity = (TopSpeed / Pi) * MotorRb.linearVelocity.normalized;
        }
        else
        {
            if (pre_accel_mul < 0)
            {
                maxrotspeed = .9f;
                if (CurrentSpeed > 10f)
                    MotorRb.linearVelocity = (10f / Pi) * MotorRb.linearVelocity.normalized;
            }
        }
        //
